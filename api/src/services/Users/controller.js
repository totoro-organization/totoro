...
module.exports = {
				...
				function (user, done) {
					if (user) {
						...
						return res
							...
							.json({ entity: Users.name, message: error.access_forbidden.message });
					}
				},
  ...
  getFavorites: function (res, id) {
    ...
    const condition = {user_id: id}
		commonsController.getAll(res, Favorites, condition, ['assos_id', 'ads_id', 'status_id'], includeFavorites);
  },

  createFavorite: async function (res, data) {
    const condition = {}
    if((!data.assos_id && !data.ads_id) || (data.assos_id && data.ads_id))
			return res
					.status(error.parameters.status)
					.json({message: "send assos_id or ads_id in data" });
    ...
    if (data.ads_id) {
      const job = await getRow(res, Ads, { id: data.ads_id });
      condition.ads_id = data.ads_id
    }
  ...
  getUserAds: async function (res, id, queries = null) {
    let condition = {};
		if(queries && queries.status){
			let statusData = await getRow(res, Status, { label: queries.status });
			condition.status_id=statusData.id;
    ...
    condition = Object.keys(condition).length === 0 ? null : condition;
    const excludeGroup = ['ads_id'];

    commonsController.getAll(res, Groups, {user_id: id}, excludeGroup, includeUser);
  },
