...
exports.router = (function () {
  ...
  terminalsRouter.post("/", [passportAdmin, async function (req, res) {
    const data = req.body;
    const condition = { label: data.label };
    const { id: status_id } = await getRow(Status, { label: label_status.disabled});
    data["status_id"] = status_id;

    controller.create(null, res, Terminals, data, condition);
  }]);

  terminalsRouter.put("/:id", [passportAdmin, async function (req, res) {
    const id = req.params.id;
    const data = req.body;
  ...
  terminalsRouter.delete("/:id", [passportAdmin, async function (req, res) {
    const id = req.params.id;
