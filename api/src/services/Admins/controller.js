...
module.exports = {
		...
		commonsController.create(
			function (result) {
				...
				sendMail(
					...
					{ to: email, subject: createAdmin.subject },
					{
	...
	getLogs: async function (res, queries) {
		...
		if (end_date && start_date && end_date < start_date)
			return res.status(error.parameters.status).json({
				entity: Logs.name,
	...
	resetPassword: async function (res, id) {
		...
		sendMail(
			resetPasswordAdmin.template,
			{ to: adminData.email, subject: resetPasswordAdmin.subject },
			{
