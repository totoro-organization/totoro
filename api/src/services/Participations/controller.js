...
module.exports = {
	...
	updateParticipation: async function (res, id, data) {
		...
		const participationData = await getRow(res, Groups, { id }, [
			{
				model: Jobs,
				as: "job",
		...
		if (statusData.label !== label_status.deleted) {
			if (
				participationData.status.label === label_status.actived &&
				statusData.label === label_status.closed
			) {
				asyncLib.waterfall(
					[
						function (done) {
							Users.update(
								{
									total_token:
										parseInt(participationData.participant.total_token) +
										parseInt(participationData.job.difficulty.token),
								},
					...
					function (Users) {
						if (Users) commonsController.update(res, Groups, id, data);
						else
							return res
				...
				if (participationData.status.label === label_status.closed) {
					return res
						...
						.json({
							entity: Groups.name,
							message: "This mission has already been validated",
						});
				} else {
			...
			asyncLib.waterfall(
				[
					function (done) {
						Jobs.update(
							{
								remaining_place:
									parseInt(participationData.job.remaining_place) + 1,
							},
				...
				function (jobs) {
					if (jobs) commonsController.delete(res, Groups, { id });
					else
						return res
							.status(error.op_failed.status)
