	...
	},
	{
		...
		include: [
			...
			},
			{
				...
				as: "status",
				attributes: excludeCommon,
				where: { label: label_status.actived },
			},
		],
	},
	{
...
module.exports = {
	getJobs: async function (res, queries) {
		...
		if (min && max)
			condition.participants_max = {
				[Op.and]: [{ [Op.gte]: parseInt(min), [Op.lte]: parseInt(max) }],
			};

		if (isExpired) {
			condition.start_date = JSON.parse(isExpired)
				? Jobs.sequelize.literal(
						`DATE_FORMAT(CURRENT_TIMESTAMP, "%Y-%m-%d") > DATE_FORMAT(Jobs.start_date, "%Y-%m-%d")`
				  )
				: Jobs.sequelize.literal(
						`DATE_FORMAT(CURRENT_TIMESTAMP, "%Y-%m-%d") <= DATE_FORMAT(Jobs.start_date, "%Y-%m-%d")`
				  );
		...
		if (latitude && longitude) {
			...
			params.attributes["include"] = [
				[Jobs.sequelize.literal(haversine), "distance"],
			];
			params.order = Jobs.sequelize.col("distance");
			if (distance)
				params.having = Jobs.sequelize.literal(`distance <= ${distance}`);
		}
	...
	createJob: async function (req, res) {
		...
		const difficultyData = await getRow(res, Difficulties, {
			id: difficulty_id,
		});
		const authorData = await getRow(res, Associations_users, {
			id: assos_user_id,
		});
	...
	updateJob: async function (res, id, data) {
		...
		if (tagsJob) {
			Tag_jobs.destroy({ where: { Jobs_id: id } })
				.then((result) => {
					var allValueTags = [];
					for (var i = 0; i < tagsJob.length; i++) {
						var valueObj = {
			...
			if (status_id || difficulty_id) {
				const { participants, job } =
					await module.exports.membersOrganisationJob(res, id);
				for (const participant of participants) {
					sendMail(
						updateJob.template,
						{ to: participant.email, subject: updateJob.subject },
						{ job, status: stat, difficulty: diff }
					);
				}
			}
			commonsController.update(res, Jobs, id, data, condition);
	...
	deleteJob: async function (res, id) {
		const { members, job } = await module.exports.membersOrganisationJob(
			res,
			id
		);
		for (const member of members) {
			sendMail(
				updateJob.template,
				{ to: member.email, subject: updateJob.subject },
				{ job }
			);
		}
	...
	registerToJob: async function (res, data) {
		...
		const jobData = await getRow(res, Jobs, { id: jobs_id }, [
			{ model: Associations_users, as: "author" },
		]);
		const condition = { jobs_id, user_id };

		if (
			jobData.remaining_place !== null &&
			parseInt(jobData.remaining_place) == 0
		) {
			return res
				.status(error.access_forbidden.status)
				.json({
					entity: Groups.name,
					message: "sorry the number of participants is full",
				});
		} else {
			asyncLib.waterfall(
				[
					function (done) {
						getField(
							res,
							Associations_users,
							{ assos_id: jobData.author.assos_id, user_id },
							done
						);
					},
				],
				function (found) {
					if (!found) {
						commonsController.create(
							async function (result) {
								const linkQrcode = await qrcode(
									path.qrcodeParticipations,
									result.id
								);
								asyncLib.waterfall(
									[
										...
										function (result, done) {
											if (jobData.remaining_place !== null) {
												Jobs.update(
													{
														remaining_place:
															parseInt(jobData.remaining_place) - 1,
													},
									...
									async function (result) {
										if (result) {
											const { members, job } =
												await module.exports.membersOrganisationJob(
													res,
													jobs_id
												);
											for (const member of members) {
												sendMail(
													registerToJob.template,
													{ to: member.email, subject: registerToJob.subject },
													{ job, user: userData }
												);
											}
											return res
											...
											return res
												...
												.json({
													entity: Groups.name,
													message: error.during_creation.message,
												});
									}
								);
							},
						...
						return res
							...
							.json({
								...
								message: "Cannot participate in a mission of your association",
							});
				}
			);
		}
	...
	membersOrganisationJob: async function (res, id) {
		const data = await getRow(res, Jobs, { id }, [
			{
				model: Associations_users,
				as: "author",
				include: [
