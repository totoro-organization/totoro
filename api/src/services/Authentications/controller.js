			...
			function (user) {
				...
				if (user.status.label !== label_status.actived) {
					...
					if (user.status.label === label_status.disabled)
						sendMail(
							verify.template,
							{ to: user.email, subject: verify.subject },
		...
		*/
		...
		commonsController.create(
			function (result) {
				...
				sendMail(
					signup.template,
					{ to: email, subject: signup.subject },
					{ firstname: result.firstname, lastname: result.lastname, token }
				);
		...
		asyncLib.waterfall(
			...
			function (found) {
				if (found) {
					...
					sendMail(
						forgot.template,
						{ to: email, subject: forgot.subject },
						{ firstname: found.firstname, lastname: found.lastname, token }
					);
