...
module.exports = {
	...
	createTransaction: async function (res, data) {
		...
		data.end_date = moment()
			.add(discountData.duration, "months")
			.format("YYYY-MM-DD");
		...
		if (parseInt(userData.total_token) < parseInt(discountData.cost)) {
			...
			commonsController.create(
				async function (result) {
					const linkBarCode = await barcode(
						path.codebareDiscounts,
						result.code
					);
					asyncLib.waterfall(
						[
							...
							function (tokens, done) {
								Users.update(
									{
										total_token:
											parseInt(userData.total_token) -
											parseInt(discountData.cost),
									},
						...
						function (Users) {
							if (Users)
								return res.status(success.create.status).json({
									entity: Tokens.name,
									message: success.create.message,
								});
							else
								return res.status(error.op_failed.status).json({
									entity: Tokens.name,
