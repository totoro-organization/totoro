...
const JobsTable: FC<TableProps<Job>> = ({
  ...
  return (
      <TableContainer>
       ...
       { jobs.length ? <Modal open={deleteModalOpen} handleClose={handleCloseDeleteModal} title={`Supprimer la mission suivante : ${deleteModalItem?.title}`}>
            <DeleteContent message={`Etes vous sûr de vouloir supprimer la mission suivante : ${deleteModalItem.title} `} handleClose={handleCloseDeleteModal} handleDelete={handleDelete} item={deleteModalItem} />
        </Modal> : null }
      </TableContainer>
  );
};
